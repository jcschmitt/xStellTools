function COMMENTS = make_merc_balloon_plots(VMEC_FILENAME, ...
    COBRA_FILENAME)
...
for ii = 1:num_configs
    ...
    try
        ...
        COMMENTS{ii} = [ 'Error on #' num2str(ii)];
        jdotbCOMMENTS{ii} = [ 'Error on #' num2str(ii)];
...
for ii = plot_index
    %keyboard
    ind_min = round( (0.1 - s{ii}(1)) / (s{ii}(2) - s{ii}(1)));
    ind_max = length(s{ii}) - 1;
    plot_indices = ind_min:ind_max;
...
for ii = plot_index
    %keyboard
    plot_indices = [2:(length(s{ii})-1)];
    renorm_factor = (1./4.) ./ vmec_data{ii}.DShear(plot_indices);
    
    subplot(2,2,1);box on;hold on;
    plot(s{ii}(plot_indices), renorm_factor .* vmec_data{ii}.DCurr(plot_indices),  'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DCurr');
        xlabel('$s = \psi_{tor} / \psi_{tor,LCFS}$', 'Interpreter', 'latex')

    subplot(2,2,2);box on;hold on;
    plot(s{ii}(plot_indices), renorm_factor .* vmec_data{ii}.DGeod(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DGeod');
        xlabel('$s = \psi_{tor} / \psi_{tor,LCFS}$', 'Interpreter', 'latex')

    subplot(2,2,3);box on;hold on;
    plot(s{ii}(plot_indices), renorm_factor .* vmec_data{ii}.DShear(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DShear');
        xlabel('$s = \psi_{tor} / \psi_{tor,LCFS}$', 'Interpreter', 'latex')

    subplot(2,2,4);box on;hold on;
    plot(s{ii}(plot_indices), renorm_factor .* vmec_data{ii}.DWell(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DWell');
...
for ii = plot_index
    ...
    subplot(1,3,3);box on;hold on;
    plot(s{ii}(plot_indices), vmec_data{ii}.DMerc(plot_indices)-vmec_data{1}.DMerc(plot_indices),  'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DMerc (rel)');
    
    subplot(2,3,1);box on;hold on;
    plot(s{ii}(plot_indices), vmec_data{ii}.DCurr(plot_indices)-vmec_data{1}.DCurr(plot_indices),  'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DCurr (rel)');
    
    subplot(2,3,2);box on;hold on;
    plot(s{ii}(plot_indices), vmec_data{ii}.DGeod(plot_indices)-vmec_data{1}.DGeod(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DGeod (rel)');
    
    subplot(2,3,4);box on;hold on;
    plot(s{ii}(plot_indices), vmec_data{ii}.DShear(plot_indices)-vmec_data{1}.DShear(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DShear (rel)');
    
    subplot(2,3,5);box on;hold on;
    plot(s{ii}(plot_indices), vmec_data{ii}.DWell(plot_indices)-vmec_data{1}.DWell(plot_indices),   'Linewidth', linewidth);
    ylim(scale_yaxis*[-1.5 1]);
    title('DWell (rel)');
    ...
    disp(['<----Failed to make relative DMerc plots']);
end
...
for ii = plot_index
    ...
    plot(rhoplot{ii}, cobradata{ii}.cobra_max_profile  , 'Marker', 'None', 'Linewidth', linewidth);
    ylabel('max(\gamma \tau)');
    xlabel('$\rho = \sqrt{\psi_{tor} / \psi_{tor,LCFS}}$', 'Interpreter', 'latex')
...
for ii = plot_index
    hold on; box on; grid on;
    plot(splot{ii}, cobradata{ii}.cobra_max_profile  , 'Marker', 'None', 'Linewidth', linewidth);
    ylabel('max(\gamma \tau)');
    xlabel('$s = \psi_{tor} / \psi_{tor,LCFS}$', 'Interpreter', 'latex')
...
for ii = plot_index
    hold on; box on; grid on;
    plot(a0{ii} * sqrt(s{ii}), vmec_data{ii}.iotaf  , 'Marker', 'None', 'Linewidth', linewidth);
    ylabel('\iota_f');
    xlabel('$r_eff  m$', 'Interpreter', 'latex')
