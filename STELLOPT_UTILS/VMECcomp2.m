...
switch datatype
    ...
    case 'iotareff'
        ...
        for i=1:nfiles
            ns=vmec_data{i}.ns;
            iota=vmec_data{i}.iotaf;
            scale_factor = vmec_data{i}.Aminor;
            if (i == 1)
                plot3(scale_factor* sqrt(0:1/(ns-1):1),i.*ones(1,ns),iota,'b','LineWidth',2.0);
            elseif (i == nfiles)
                plot3(scale_factor* sqrt(0:1/(ns-1):1),i.*ones(1,ns),iota,'r','LineWidth',2.0);
            else
                plot3(scale_factor* sqrt(0:1/(ns-1):1),i.*ones(1,ns),iota,'k');
            end
            out = scale_factor* sqrt(0:1/(ns-1):1);
            out2 = iota;
            [out' out2']
        end
        %xlabel('Normalized Flux');
    ...
    case 'iotatsratphi0'
        ...
        for i=1:nfiles
            ...
            R0atphi0 = sum(vmec_data{i}.rmnc);
            if i == 1
                plot3(R0atphi0,i.*ones(1,ns),iota,'b','LineWidth',2.0);
            elseif (i == nfiles)
                plot3(R0atphi0,i.*ones(1,ns),iota,'r','LineWidth',2.0);
            else
                plot3(R0atphi0,i.*ones(1,ns),iota,'k');
            end
            out = R0atphi0;
            out2 = iota;
            [out' out2']
        ...
        %xlabel('Normalized Flux');
        xlabel('Outboard Radius at \Phi=0 (m)');
        set(gca,'YTick',1:nfiles);
        set(gca,'YTickLabel',filename);
        zlabel('Rotational Transform');
        view(3);
...
%         axis equal
    ...
    case 'fluxpi2'
        ...
        for i=1:nfiles
            ...
            %surfs2plot = 10:10:ns;
            for j=surfs2plot
                plot3(r(j,:),i.*ones(1,ntheta),z(j,:),'Color',color);
            end
            plot3(r(1,1),i,z(1,1),'+k');
            plot3(r(ns,:),i.*ones(1,ntheta),z(ns,:),'Color',color);
        end
        xlabel('R [m]');
        set(gca,'YTick',1:nfiles);
        set(gca,'YTickLabel',filename);
