function pcurr = sum_cossq_sqrts(x, ac_array)
...
for jj = 1:length(x)
    ...
    for ii =1:ncssq  % ! check the interval of x in ( bsta(i) : bend(i) ]
        if ( (this_sqx <= bend(ii)) && (this_sqx > bsta(ii)) )
            ni = ii;
        end
    end
    ...
    for ii=1:ni
        if ( (this_sqx > bsta(ii)) && (this_sqx <= bend(ii)) )
            if(ii == 1)   % ! the first has no linear term.
                pcurr(jj) = pcurr(jj) + ac_array(ii+1) * (0.25*this_x+   ...
                    (delx/(2*pi)*this_sqx*sin(pi*this_sqx/delx)) +  ...
                    delxsq/(2*pisq)*(cos(pi*this_sqx/delx) - 1.0));
            else
                pcurr(jj) = pcurr(jj) + ac_array(ii+1)*(0.25*this_x+         ...
                    delx / (2*pi)*this_sqx*sin(pi*(this_sqx-xi(ii)) / delx)+    ...
                    delxsq / (2*pisq) * cos(pi*(this_sqx-xi(ii)) / delx)-     ...
                    0.25*bsta(ii)^2 + delxsq / (2*pisq) );
            end
        else
            if (ii == 1) %   !the first is only a half interval
                % !           if(x .gt. 0.0_dp )
                % !    &          pcurr=pcurr+ac(i)*(0.25_dp-1/pisq)*delxsq
                if this_x > 0
                    pcurr(jj) = pcurr(jj) + ac_array(ii+1) * (0.25 - 1.0 / pisq) * delxsq;
                end
            else
                pcurr(jj) = pcurr(jj) + ac_array(ii+1) * delx * xi(ii);
            end
        end
    end
