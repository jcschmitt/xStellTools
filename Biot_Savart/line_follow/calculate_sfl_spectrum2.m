...
BFFT_gw_imag_scaled =  B_FFT_gw_even_real_half * scale_gw2orig;
...
while keep_going
    switch task
        case 'find00'
            ...
            [fit_results{loop_index}, resnorm, residual, exitflag] = ...
                lsqcurvefit(@gaussCurveFit, gauss_init_guess, x_fit, y_fit, [-Inf -1 0], [+Inf 1 1], gauss_fit_options);
            data_fit = gaussCurveFit(fit_results{loop_index}, n_minus_miota);
            
            data_new = data_current - data_fit;
            search_mask_min = 1;
            search_mask_max = ceil(mask_mult*(fit_results{loop_index}(3) / delta_nmmi));
        ...
        case 'findsym'
            ...
            gauss_init_guess = [this_max, n_minus_miota(index_max), fit_results{1}(3)]; % the initial guess
            ...
            search_mask_min = index_max - ceil(mask_mult*(fit_results{loop_index}(3) / delta_nmmi));
            search_mask_max = index_max + ceil(mask_mult*(fit_results{loop_index}(3) / delta_nmmi));
            search_mask(search_mask_min:search_mask_max) = 0;
    ...
    if DEBUG
        ...
        this_result = fit_results{loop_index}
