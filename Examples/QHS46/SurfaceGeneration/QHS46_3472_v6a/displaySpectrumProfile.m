function [] = displaySpectrumProfile(folder, spectrumType, surfs, ModesToDisplay, COLOR_SPEC, STYLE_SPEC, XAXIS)
%
...
if (nargin < 3)
    error('Please enter surfaces to display');
end

SHOW_LABELS = 1;

disp(['Displaying radial profile of ' spectrumType ' spectrum of ' folder]);

numSurfs = length(surfs);

for ii = 1:numSurfs
    filename = ['LineFollow_' spectrumType '_Spectrum_Data_surface_' num2str(surfs(ii)) '.mat'];
...
[sorted_crap, sorted_indices] = sort(max_of_mode, 'descend');
