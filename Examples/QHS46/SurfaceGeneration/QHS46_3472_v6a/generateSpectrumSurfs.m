...
for ii = surfaceToGenerate
    ...
    filename = ['LineFollow_' spectrumType '_Spectrum_Data_surface_' num2str(ii)];
    try
        load(filename);
    ...
    for jj = startingBlock:transitBlocks
        ...
        [chiBlock, coordsBlock] = ...
            LineFollow_QHS46_34742_v6a_Spectrum(spectrumType, current, ...
    ...
    elseif strcmp(lower(spectrumType), 'boozer')
        ...
        if ~(exist('g_Boozer'))
            g_Boozer = calculateBoozer_gFactor(current, rStart(1), relTol, absTol);  % rStart(1) should be the magnetic axis
        else
    ...
    if 1
        ...
        [nm_amp, pk_n_sorted, pk_m_sorted, pk_pos_sorted, nm_avail, ...
            nm_error, nm_next_best_error, n_values, m_values, iota_best] = ...
            calculateSpectrum(chi_FFT, modB, spectrumType, dV_dPsi, ...
            g_Boozer, iota_pp, DEBUG);  %#ok<ASGLU>
