...
for jj = 2:length(pk_pos_sorted)  % skip first one
    ...
    nm_mismatch_next_best = abs(abs(nm_pos_value) - abs(pk_pos_sorted(jj))) .* nm_avail;  % If nm_avail(n,m) == Inf, then, mismatch = Inf
    % find the (n, m)-indices of the minimum mismatch value
