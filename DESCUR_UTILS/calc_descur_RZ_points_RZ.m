function [surf_x, surf_y, surf_z, phi_out] = ...
    calc_descur_RZ_points(coilsetID, rboundary, zboundary, nphi, ntheta, coil_current_array)
% function [surf_x, surf_y, surf_z, phiout] = ...
%     calc_descur_RZ_points(coilsetID, rboundary, nphi, ntheta, current)
% inputs:
%   coilsetID- The name of the coilset for line-following
...
[phi_out, x_out] = field_line_follow(coilsetID, coil_current_array, ...
    r_start, z_start, phi_start, phi_inc, phi_end, rel_tol, abs_tol);

% Extract the data into usful arrays
r_surf = x_out(:,1);
z_surf = x_out(:,2);
surf_x = [r_surf.*cos(phi_out)]';
surf_y = [r_surf.*sin(phi_out)]';
surf_z = [z_surf]';
...
% z = x(2);
% 
% [bx, by, bz] = calc_b_QHS46_RPhiZ(r, phi, z, coil_current_array);
% 
