function [outfilename, descurcmdfilename] = make_descur_file(LCFS_filename, LCFS_pathname, num_field_periods, N_phi, N_theta)
% function [] = make_descur_file(LCFS_filename, LCFS_pathname)
...
try
    ...
    descurcmdfilename = ['descur_' LCFS_filename(6:(end-4)) '.cmd'];
    ...
    fprintf(fid,'%s %s %s \n',num2str(ntheta),num2str(nphi),num2str(num_field_periods));
    ...
    fid2 = fopen(descurcmdfilename, 'w');
    fprintf(fid2,'4\nV\n0\n%s\nN\n', outfilename);
    fclose(fid2);
    
    cd(cur_pwd);
catch
    warning('error in make_descur_file.m');
    LCFS_filename
    LCFS_pathname
    cd(cur_pwd);
    fclose(fid);
